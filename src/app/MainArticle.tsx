...
const MainArticle = ({ recentData }: { recentData: BlogDataType }) => {
  return (
    <div className="w-11/12 md:w-6/12 mx-auto mt-16 mb-32 relative group animate-fadeUp">
      <h2 className="text-4xl font-bold mb-2 text-orange-600">Recent Topic</h2>
      <Link href={`selectBlogPage/${recentData?.id}`}>
        <img src={recentData?.eyecatch?.url} alt={''} className="rounded-md animate-fadeIn" />
      </Link>
